create table asset_entity (money_value numeric(38,2), type smallint check (type between 0 and 1), id uuid not null, summary_id uuid, name varchar(255), primary key (id));
create table asset_entity_item_entities (asset_entity_id uuid not null, item_entities_id uuid not null unique);
create table customer (customer_id uuid not null, customer_stripe_id varchar(255), email varchar(255) not null, payment_method_id varchar(255), stripe_subscription_id varchar(255), primary key (customer_id));
...
create table password_reset_token (expiry_date timestamp(6), id bigserial not null, user_id uuid not null unique, token varchar(255), primary key (id));
...
create table summary_entity (money_value numeric(38,2), state smallint check (state between 0 and 2), date timestamp(6), id uuid not null, user_id uuid not null, primary key (id));
...
create table users_summary (currency smallint check (currency between 0 and 2), user_id varchar(255) not null, primary key (user_id));
create table user_subscription (expires date, start date, status smallint check (status between 0 and 1), subscription_type smallint check (subscription_type between 0 and 0), subscription_id uuid not null, user_user_id uuid unique, primary key (subscription_id));
create table verification_tokens (expiry_date timestamp(6), id bigserial not null, user_id uuid not null, token varchar(255), primary key (id));
